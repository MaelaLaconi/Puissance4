...
// Definition d'un état du jeu (état/position du jeu)
...
typedef struct {
    // un coup se definie par une colonne)
    int colonne; //colonne dans laquelle on joue
...
Etat *etat_initial () {
    Etat *etat = (Etat *)malloc(sizeof(Etat)); //création de l'état
     ...
     // grille 7 lignes, 6 colonnes vides
    int i,j;
    for (i = 0; i < 7; i++)
        for (j = 0; j < 6; j++)
            etat->plateau[i][j] = ' ';
...
Coup *demanderCoup () {
    ...
    int i = -1; // on le met à -1 car si le user n'ecrit pas un entier le programme redemandera de choisir
...
int coupJouable (Etat * etat, Coup * coup) {
    //parcours des lignes pour trouver la première libre, si aucune sur la hauteur de la plateau le coup n'est pas jouable
    if (coup->colonne >= 0 && coup->colonne < 7)
        for (int j = 0; j < 6; j++)
            if (etat->plateau[coup->colonne][j] == ' ')
                return 1;
...
void jouerCoup (Etat *etat, Coup *coup) {
    ...
    for (j = 0; j < 6; j++) {
        //Si on n'a pas déja un jeton a l'emplacement de notre coup
        if (etat->plateau[coup->colonne][j] == ' ') {
            etat->plateau[coup->colonne][j] = etat->joueur ? 'O' : 'X';
            break;
        }
    }
...
Coup **coups_possibles (Etat *etat) {
    ...
    for(i = 0; i < 7; i++) {
        ...
        //si le coup en colonne i est jouable on l'ajoute à la liste des coups jouables
            ...
            coups[k] = coup; // on ajoute le coup
            k++;
        } else {
            free(coup);
        }
    ...
    for (i = 0; i < 7; i++) {
        for(j = 0; j < 6; j++) {
            if (etat->plateau[i][j] != ' ') {
                ...
                // diagonales
                k = 0;
                while (k < 4 && j+k < 6 && i+k < 7 && etat->plateau[i+k][j+k] == etat->plateau[i][j])
                    k++;
                if (k == 4) // si on a 4 jetons d'affilé sur la meme diagonale 
                    return etat->plateau[i][j] == 'O'? ORDI_GAGNE : HUMAIN_GAGNE;
                
                k = 0;
                while (k < 4 && j-k >= 0 && i+k < 7 && etat->plateau[i+k][j-k] == etat->plateau[i][j])
                    k++;
                if (k == 4) // si on a 4 jetons d'affilé sur la meme diagonale
                    return etat->plateau[i][j] == 'O'? ORDI_GAGNE : HUMAIN_GAGNE;
            }
...
double B_Value(Noeud * noeud){
    ...
    return (signe * moyenne) + (c * sqrt(log(noeud->parent->nb_simus)/noeud->nb_simus));
}
...
void ordijoue_mcts(Etat * etat, int criteres, clock_t tempsmax) {
...
int main (int argc, char **argv) {
    ...
    if (argc < 2 || argc > 3) {
    	printf("Utilisation : ./jeu [temps] (option : [debug=1])\n");
    	return 0;
    }
    ...
    do {
        ...
        printf("Quel critère utiliser ( 0 : Max, 1 : Robuste ) ? ");
        scanf("%d", &criteres);
        
    } while (criteres < 0 || criteres > 1);
    ...
    while (fin == NON) {
        ...
        if ( etat->joueur == 0 ) {
            ...
            ordijoue_mcts( etat, criteres, TEMPS );
        }
